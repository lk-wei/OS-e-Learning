...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = mysqli_prepare($conn, "SELECT * FROM student WHERE S_Email =?");
    if (!$stmt) {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (password_verify($password, $user['S_Password'])) {
            ...
            $_SESSION['password'] = $user['S_Password'];
            $_SESSION['contactnumber'] = $user['S_Contact_Number'];
            $_SESSION['guardianId'] = $user['Guardian_ID'];
        ...
        <div class="form-box login">
            ...
            <form action="loginstudent.php" method="post">
                ...
                <div class="login-register">
                    <p>Don't have an account?<a href="registerstudent.php" class="register-link"> Register</a></p>
                </div>
            </form>
        </div>
    </div>
