<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $contactnumber = mysqli_real_escape_string($conn, $_POST['L_Contact_Number']);

        // Prepare an insert statement
        $sql = "INSERT INTO lecturer (L_Username, L_Email, L_Password, L_Contact_Number) VALUES (?, ?, ?, ?)";
    
        if($stmt = mysqli_prepare($conn, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssss", $param_username, $param_email, $param_password, $param_contactnumber);
